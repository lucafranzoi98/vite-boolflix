...
<template>
   ...
   <div v-if="state.isEmpty == false">
      <ul v-if="state.movies != ''">Movies
         <li v-for="movie in state.movies">{{ movie.title }}, {{ movie.original_title }}, {{ state.getFlagEmoji(movie.original_language) }}, {{ movie.vote_average }}, {{ movie.backdrop_path }}
         <img v-if="movie.backdrop_path != null" :src="state.getCover(movie.backdrop_path)">
         <img v-else src="./assets/img/404.png" width="100"></li>
      </ul>
      <div v-else>No movies found</div>

      <ul v-if="state.series != ''">Series
         <li v-for="serie in state.series">{{ serie.name }}, {{ serie.original_name }}, {{ state.getFlagEmoji(serie.original_language) }}, {{ serie.vote_average }}
